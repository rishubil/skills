...
Once a Field is created with the [createField()](https://developer.garmin.com/connect-iq/api-docs/Toybox/ActivityRecording/Session.html#createField-instance_function) method, you can submit the next Field value with [setData()](https://developer.garmin.com/connect-iq/api-docs/Toybox/FitContributor/Field.html#setData-instance_function), which will get written to the FIT file at the next opportunity. Depending on the device, writes to the FIT file may occur once per second or when new data is available (Smart Recording). Best practice is to only call [setData()](https://developer.garmin.com/connect-iq/api-docs/Toybox/FitContributor/Field.html#setData-instance_function) when values have changed to accommodate Smart Recording.

If [setData()](https://developer.garmin.com/connect-iq/api-docs/Toybox/FitContributor/Field.html#setData-instance_function) is called before the previous data is written out, the previous value will be lost and replaced by the current data. For this reason, we do not recommend using this feature for time-sensitive data requiring sub-second granularity.
...
- [**setData**](#setData-instance_function "setData (Instance Function)") (input as [Lang.Object](https://developer.garmin.com/connect-iq/api-docs/Toybox/Lang/Object.html)) as **Void**
