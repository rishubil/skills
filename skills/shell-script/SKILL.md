---
name: shell-script
description: "Specialist skill for writing Bash shell scripts. Applies best practices and error handling patterns. Use when writing new bash scripts (.sh/.bash), improving or reviewing existing scripts, adding error handling, or editing files in the scripts/ folder."
---

# Shell Script Best Practices

> [!NOTE]
> Original: https://sharats.me/posts/shell-script-best-practices/
...
10. Always quote variable accesses with double-quotes.
...
11. Use `local` variables in functions.
12. Accept multiple ways that users can ask for help and respond in kind.
...
14. When printing error messages, please redirect to stderr.
    - Use `echo 'Something unexpected happened' >&2` for this.
15. Use long options, where possible (like `--silent` instead of `-s`). These serve to document your commands explicitly.
    - Note though, that commands shipped on some systems like macOS don't always have long options.
16. If appropriate, change to the script's directory close to the start of the script.
    - And it's usually always appropriate.
    - Use `cd "$(dirname "$0")"`, which works in _most_ cases.
17. Use `shellcheck`. Heed its warnings.
