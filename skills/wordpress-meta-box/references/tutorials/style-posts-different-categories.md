...
- Add a class to the body tag (via the body\_class filter) to customize the CSS for the whole page more conveniently;
...
- `get_term_meta ($ categories [0] -> term_id, 'background_color', true)`: use `get_term_meta ()` to get the value of the custom field corresponding to the category returned by the get\_the\_category() function, with the ID of the custom field is `background_color` (we created it in step 1).
