...
- [MB Views](https://metabox.io/plugins/mb-views/): Build front-end templates for WordPress without touching theme files. Support Twig and all field types.

If you are using a page builder such as [Elementor](https://metabox.io/plugins/mb-elementor-integrator/) or [Beaver Builder](https://metabox.io/plugins/mb-elementor-integrator/), please activate the corresponding integration from Meta Box.
...
First, we need custom fields for saving URLs for each post. If you’ve had [MB Builder](https://metabox.io/plugins/meta-box-builder/) in your pocket, you will have a UI on the backend to create custom fields visually. Otherwise, please go to the [Online Generator](https://metabox.io/online-generator/), create fields, generate code, and copy it to get the fields.
...
In there, `view_demo` and `download` are custom fields with IDs. By the way, because I’m using the [eStar](https://gretathemes.com/wordpress-themes/estar/) theme, I must use the `wp-block-button__link` class to create buttons.
...
I’ve already pre-made a template using [Oxygen](https://oxygenbuilder.com/). It is applied to blog posts.
...
I also created a similar template with [Beaver Builder](https://www.wpbeaverbuilder.com/).
...
The last page builder is [Bricks](https://bricksbuilder.io/). In the same way, add a container with 4 columns first, then buttons.
