...
The last one is to set a secret key (unique\_key) in the URL. You can set it on your own. This means that all the actions in this function run only when the secret key is run.
...
So, you have to access the URL in this form: **domain.com/?unique\_key**. This action helps to run the secret key, then the code in the.php file will run to convert the data.
