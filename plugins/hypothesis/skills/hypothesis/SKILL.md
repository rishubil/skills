...
name: hypothesis
description: >
  Write property-based tests for Python code using the Hypothesis library,
  following a structured explore → identify → write → run workflow.
  Use when the user asks to add Hypothesis tests, find testable properties,
  or write property-based tests for a Python module, file, or function.
---
...
Take the scope from the user's message: if the user named a specific module, file, or function, focus there; otherwise explore the entire codebase.
