...
I’ll create a template for the page using both two ways: 1.\*\* Adding code to the theme’s files\*\*: you should add some files to the theme’s folder as well as add code to several different files. 2. **Using the MB Views extension of Meta Box**: just work on a place only for all. Besides that, the template will not be affected when you change the theme.
...
And then, have a loop to get data from all the posts, and assign them to the array using this: `{% set restaurantsArray = restaurantsArray|merge(`.
