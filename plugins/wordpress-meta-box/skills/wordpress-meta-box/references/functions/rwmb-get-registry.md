...
| --- | --- |
| `$field_id` | Field ID |
| `$sub_type` | The sub-type of the field.   For posts, sub-type is the post type slug.   For terms, it's the taxonomy slug.   For users, it's always `user`.   For settings, it's the option name. |
